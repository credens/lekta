...
struct CheckoutView: View {
    ...
    var body: some View {
        ...
        .onChange(of: paymentMethod) { method in
            cancelPolling()
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    HStack {
                        ...
                        TextField("0", text: $montoRecibido)
                            ...
                            .foregroundStyle(.mpBrown)
                            .focused($cashFocused)
                            .onChange(of: montoRecibido) { v in
    ...
    private var successOverlay: some View {
        ZStack {
            ...
            VStack(spacing: 20) {
                ...
                if paymentMethod == .cash, let recibido = Double(montoRecibido), vuelto > 0 {
        ...
        pollingTask = Task {
            while !Task.isCancelled {
                ...
                if let status = try? await MercadoPagoService.checkPaymentStatus(preferenceId: preferenceId) {
                    ...
                    if status == .approved {
                        await MainActor.run { Task { await confirmPayment() } }
                        break
                    }
                }
            }
