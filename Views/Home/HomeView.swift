...
struct HomeView: View {
            ...
            .alert("Cerrar Caja", isPresented: $showCerrarCajaConfirm) {
                ...
                Button("Cerrar", role: .destructive) {
                    Task {
                        ...
                        if cajaVM.ultimoResumen != nil { showResumen = true }
                    }
                }
            } message: {
                ...
                ToolbarItem(placement: .topBarTrailing) {
                    Menu {
                        Button(role: .destructive) {
                            ...
                            Label("Desconectar MercadoPago", systemImage: "link.badge.minus")
                        }
                    } label: {
    ...
    private var cajaBadge: some View {
        ...
        case .abierta(let desde):
            Label("Caja abierta desde \(desde.formatted(.dateTime.hour().minute()))", systemImage: "checkmark.circle.fill")
                .font(.system(.caption, design: .rounded).weight(.semibold))
                .foregroundStyle(.white)
                .padding(.horizontal, 14)
                .padding(.vertical, 6)
...
struct HomeActionButton: View {
    ...
    var body: some View {
        Button(action: action) {
            ...
            .foregroundStyle(isDisabled ? AnyShapeStyle(Color.secondary) : AnyShapeStyle(fgColor))
